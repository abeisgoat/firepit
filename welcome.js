...
(async () => {
  const line = new Array(process.stdout.columns).fill(undefined).map(() => "#").join("");
  const thin_line = new Array(process.stdout.columns).fill(undefined).map(() => "-").join("");
  console.log(line);
  console.log("");
    ...
    if (isSetup) {
      ...
      return new Promise((resolve) => {
        ...
        install.stderr.on("data", (buf) => {
          ...
          const line = (buf.toString().split("\n").filter((l) => l).slice(-1)[0] || "").trim().slice(0, process.stdout.columns - 5);
        ...
        install.on("exit", () => {
          ...
          process.stdout.write(`${chalk.green("+")} Alright, ${chalk.green("your CLI is set up")}!\n`);
          resolve();
        });
      });
    }
...
async function CheckIsLoggedIn() {
  ...
  return new Promise((resolve) => {
    ...
    listSpawn.on("close", () => {
      ...
      if (!isLoggedIn) {
        finish("🚫", `${chalk.cyan("~")} Looks like you're not authenticated. ${chalk.yellow("Let's log in")}!\n`);
      }else {
        finish("", "");
      }
